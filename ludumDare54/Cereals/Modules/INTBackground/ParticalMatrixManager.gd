...
func MatrixBoom(position :Vector2,type:int,ten:int = 0):
	if(_curMatrix!=null):
		ParticalGenerator.MatrixBoom(position,type,ten,_curMatrix.SlotList,_curMatrix)

func MatrixParticalChange(position,type:int,particalName:String,sliceType:String = ""):
	if(_curMatrix!=null):
		ParticalGenerator.MatrixParticalChange(position,type,particalName,_curMatrix.Slice(sliceType),_curMatrix)
