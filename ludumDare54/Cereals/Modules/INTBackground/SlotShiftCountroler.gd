...
func _randSliceInLine(l:bool,ll:bool)->Array:
	var t = (UFM.RandomInt(2,_getK()-3)/2)*2
	if l:
		t = (UFM.RandomInt(2,_getK()-3)/2)*2+1
	return _KSlice(t,ll)
	

func _KSlice(k:int,it:bool)->Array:
	var res = []
	if it:
		for i in _getL(k):
			if i%2==0:
				res.append(_getInKL(k,i))
	else:
		for i in _getL(k):
			if i%2!=0:
				res.append(_getInKL(k,i))
